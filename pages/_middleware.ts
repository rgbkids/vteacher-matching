...
export default function middleware(req: NextRequest) {
  ...
  const appHost = hostname.split(".").reverse().pop();
  const appWildcardDomain = hostname.replace(!!appHost ? appHost : "", "");
  ...
  if (!pathname.includes(".") && !pathname.startsWith("/api")) {
    if (currentHost == "app" || currentHost == appHost) {
      if (
        pathname === "/login" &&
